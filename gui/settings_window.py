...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def __init__(
        self, parent: ctk.CTk, setting: Settings
    ):  # Type hint parent as ctk.CTk
    ...
    def _setup_icon(self):
        ...
        if parent_icon_path_ico and os.path.exists(parent_icon_path_ico):
            try:
                ...
                LOGGER.debug(
                    f"SettingsWindow 尝试设置 wm_iconbitmap: {parent_icon_path_ico}"
                )
            except tk.TclError as e_wm_ico:
                LOGGER.warning(
                    f"SettingsWindow 设置 wm_iconbitmap 时发生 TclError: {e_wm_ico}"
                )
            except Exception as e_wm_ico_gen:
                LOGGER.warning(
                    f"SettingsWindow 设置 wm_iconbitmap 失败: {e_wm_ico_gen}",
                    exc_info=True,
                )
        else:
            LOGGER.debug(
                "SettingsWindow: 未从父窗口获取到有效的 .ico 路径用于 wm_iconbitmap。"
            )
        ...
        if parent_icon_photo and isinstance(parent_icon_photo, ImageTk.PhotoImage):
            try:
                ...
                LOGGER.error(
                    f"SettingsWindow 安排 iconphoto 设置失败: {e_schedule}",
                    exc_info=True,
                )
        else:
            LOGGER.debug(
                "SettingsWindow: 未从父窗口获取到有效的 PhotoImage 对象用于 iconphoto。"
            )
    ...
    def create_widgets(self):
        ...
        label_client_size = ctk.CTkLabel(
            main_frame, text=self.settings.lan().CLIENT_SIZE
        )
        ...
        ms_url_entry = ctk.CTkEntry(
            main_frame, textvariable=self.ms_url_var, width=std_wid * 30
        )
        ms_url_entry.grid(row=cur_row, column=1, columnspan=2, sticky="w", **pad_args)
        self.enable_extension_var = ctk.BooleanVar(
            value=self.settings.enable_chrome_ext
        )
        ...
        mitm_port_entry = ctk.CTkEntry(
            main_frame, textvariable=self.mitm_port_var, width=std_wid * 15
        )
        mitm_port_entry.grid(row=cur_row, column=1, sticky="w", **pad_args)
        label_upstream_proxy = ctk.CTkLabel(
            main_frame, text=self.settings.lan().UPSTREAM_PROXY
        )
        ...
        self.language_var = ctk.StringVar(
            value=LAN_OPTIONS[self.settings.language].LANGUAGE_NAME
        )
        ...
        proxy_inject_checkbox.grid(
            row=cur_row, column=2, columnspan=2, sticky="w", **pad_args
        )
        cur_row += 1
        separator1 = ctk.CTkFrame(
            main_frame, height=1, fg_color=("gray70", "gray30")
        )  # Use a Frame for separator
        ...
        cur_row += 1
        label_ai_model_file = ctk.CTkLabel(
            main_frame, text=self.settings.lan().AI_MODEL_FILE
        )
        ...
        model_file_menu = ctk.CTkOptionMenu(
            main_frame,
            variable=self.model_file_var,
            values=model_files,
            width=std_wid * 30,
        )
        model_file_menu.grid(
            row=cur_row, column=1, columnspan=3, sticky="w", **pad_args
        )
        cur_row += 1
        label_ai_model_file_3p = ctk.CTkLabel(
            main_frame, text=self.settings.lan().AI_MODEL_FILE_3P
        )
        ...
        model_file_3p_menu = ctk.CTkOptionMenu(
            main_frame,
            variable=self.model_file_3p_var,
            values=model_files,
            width=std_wid * 30,
        )
        model_file_3p_menu.grid(
            row=cur_row, column=1, columnspan=3, sticky="w", **pad_args
        )
        cur_row += 1
        label_akagi_ot_url = ctk.CTkLabel(
            main_frame, text=self.settings.lan().AKAGI_OT_URL
        )
        ...
        akagi_ot_url_entry.grid(
            row=cur_row, column=1, columnspan=3, sticky="w", **pad_args
        )
        cur_row += 1
        label_akagi_ot_apikey = ctk.CTkLabel(
            main_frame, text=self.settings.lan().AKAGI_OT_APIKEY
        )
        ...
        mjapi_url_entry = ctk.CTkEntry(
            main_frame, textvariable=self.mjapi_url_var, width=std_wid * 41
        )
        mjapi_url_entry.grid(
            row=cur_row, column=1, columnspan=3, sticky="w", **pad_args
        )
        cur_row += 1
        ...
        mjapi_user_entry = ctk.CTkEntry(
            main_frame, textvariable=self.mjapi_user_var, width=std_wid * 10
        )
        mjapi_user_entry.grid(row=cur_row, column=1, sticky="w", **pad_args)
        cur_row += 1  # MJAPI Secret should be on a new row
        label_mjapi_secret = ctk.CTkLabel(
            main_frame, text=self.settings.lan().MJAPI_SECRET
        )
        ...
        self.mjapi_model_select_var = ctk.StringVar(
            value=self.settings.mjapi_model_select
        )
        ...
        label_login_to_refresh = ctk.CTkLabel(
            main_frame, text=self.settings.lan().LOGIN_TO_REFRESH
        )
        ...
        cur_row += 1
        label_random_choice = ctk.CTkLabel(
            main_frame, text=self.settings.lan().RANDOM_CHOICE
        )
        ...
        reply_emoji_menu = ctk.CTkOptionMenu(
            main_frame,
            variable=self.reply_emoji_var,
            values=reply_emoji_options,
            width=std_wid * 10,
        )
        ...
        self.delay_random_lower_var = ctk.DoubleVar(
            value=self.settings.delay_random_lower
        )
        ...
        self.delay_random_upper_var = ctk.DoubleVar(
            value=self.settings.delay_random_upper
        )
        ...
        label_settings_tips = ctk.CTkLabel(
            main_frame, text=self.settings.lan().SETTINGS_TIPS, justify="center"
        )
        label_settings_tips.grid(
            row=cur_row, column=0, columnspan=4, sticky="w", **pad_args
        )
        # endregion
        ...
        button_frame.pack(
            side=tk.BOTTOM, fill=tk.X, padx=20, pady=(10, 20)
        )  # Adjusted padding
        ...
        try:  # Add try-except around parsing/validation
            ...
            if not self.settings.valid_mitm_port(mitm_port_new):
                messagebox.showerror(
                    "⚠", self.settings.lan().MITM_PORT_ERROR_PROMPT, parent=self
                )  # Specify parent
                return
            ...
            if self.settings.language != language_new:
                ...
                self.gui_need_reload = (
                    False  # Ensure flag is reset if language didn't change
                )
            ...
            randomized_choice_new: int = int(
                self.randomized_choice_var.get().split(" ")[0]
            )
            ...
            try:
                ...
                messagebox.showerror(
                    "⚠",
                    self.settings.lan().RANDOM_DELAY_RANGE + " (输入无效)",
                    parent=self,
                )
                return
            delay_lower_new = max(0.0, delay_lower_new)  # Ensure float comparison
            delay_upper_new = max(
                delay_lower_new, delay_upper_new
            )  # Ensure upper >= lower
