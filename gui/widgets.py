...
from typing import Callable, Dict, Tuple, Union, Optional # Added Optional, Dict, Tuple, Union
...
from common.utils import sub_file, Folder # Removed unused sub_file unless needed elsewhere
...
class ToggleSwitch(ctk.CTkFrame):
    ...
    def __init__(self, master, text:str, height:int, font_size:int = 12, command:Callable=None):
        super().__init__(master, height=height, width=height*2, fg_color="transparent")
        ...
        self.img_label.bind("<Enter>", lambda e: self.img_label.configure(cursor="hand2"))
        self.img_label.bind("<Leave>", lambda e: self.img_label.configure(cursor=""))

    def _load_image(self, image_path: Union[Path, str], key: str, size: Tuple[int, int]):
        ...
        try:
            ...
            LOGGER.error(f"Failed to load ToggleSwitch image '{image_path}' as CTkImage: {e}")
    ...
    def _on_click(self, _event=None):
        ...
        if self._command:
            try:
                ...
                LOGGER.error(f"Error executing ToggleSwitch command: {e}", exc_info=True)
...
class Timer(ctk.CTkFrame):
    ...
    STOP = "⏹️"  # Stop symbol
    def __init__(self, master: ctk.CTkFrame, label_text: str, height: int = 40, default_font_size:int=12):
        ...
        self.pack_args = {'side': tk.LEFT, 'padx': (1, 1), 'pady': (1, 1)} # Common packing args
        ...
        ctk.CTkLabel(self.frame_entries, text=":", font=self.entry_font, width=5).pack(**self.pack_args)
        ...
        # Start/Stop button
        self.the_btn = ctk.CTkButton(self, text=Timer.START, font=self.button_font,
                                     command=self._toggle_timer, width=40,)
    ...
    def _setup_entry(self, parent, var: tk.StringVar, max_val: int):
        ...
        entry.bind("<FocusOut>", lambda event, sv=var, mv=max_val: self._validate_and_format_final(sv, mv), add='+')
        entry.bind("<Return>", lambda event, sv=var, mv=max_val: self._validate_and_format_final(sv, mv), add='+')
    ...
    def _validate_and_format_final(self, var: tk.StringVar, max_val: int):
        ...
        try:
            ...
            if current_value.isdigit(): # Process only if it contains digits
                 ...
                 clamped_val = min(max(0, val), max_val) # Clamp value between 0 and max_val
                 final_value = f"{clamped_val:02}" # Format with leading zero
            elif current_value: # Log if non-empty and non-digit
                 LOGGER.debug(f"Invalid non-digit value '{current_value}' during final validation. Clearing.")
            ...
            if var.get() != final_value:
             ...
             LOGGER.warning(f"Invalid non-integer value '{current_value}' during final validation. Clearing.")
             if var.get() != "": var.set("")
        except Exception as e:
             LOGGER.error(f"Unexpected error during final validation for '{current_value}': {e}")
             if var.get() != "": var.set("") # Attempt to clear on unexpected errors
        finally:
             self._validating = False
    ...
    def _start_timer(self):
        ...
        try:
            ...
            if self.the_btn.winfo_exists(): self.the_btn.configure(text=Timer.STOP)
            LOGGER.info(f"Timer started for {hours:02d}:{minutes:02d}:{seconds:02d}")
            self._stop_time = time.time() + total_seconds
            for e in self.entries:
                if e.winfo_exists(): e.configure(state="disabled")
            self._run_timer() # Start the timer loop

        except ValueError:
            # This should ideally not happen if validation worked, but log just in case
            LOGGER.error("Invalid time input format encountered when starting timer (should have been caught by validation).")
        except Exception as e:
    ...
    def _run_timer(self):
        ...
        if not self._timer_running or self._stop_time is None: return
        ...
        try:
            ...
            if remaining_time_int > 0:
                ...
                if not self._validating: # Check validation flag to prevent interference
                     if self.hour_var.get() != f"{hours:02}": self.hour_var.set(f"{hours:02}")
                     if self.minute_var.get() != f"{minutes:02}": self.minute_var.set(f"{minutes:02}")
                     if self.second_var.get() != f"{seconds:02}": self.second_var.set(f"{seconds:02}")
                ...
                if not self._validating:
                    if self.hour_var.get() != "00": self.hour_var.set("00")
                    if self.minute_var.get() != "00": self.minute_var.set("00")
                    if self.second_var.get() != "00": self.second_var.set("00")
    ...
    def _clear_time(self):
        ...
        try:
            ...
            if self.hour_var.get() != "": self.hour_var.set("")
            if self.minute_var.get() != "": self.minute_var.set("")
            if self.second_var.get() != "": self.second_var.set("")
        finally:
            self._validating = False
    ...
    def _stop_timer(self, finished=False):
        ...
        # Update button text
        if self.the_btn.winfo_exists(): self.the_btn.configure(text=Timer.START)

        # Re-enable entry fields
        for e in self.entries:
            if e.winfo_exists(): e.configure(state="normal")
...
class ToolBar(ctk.CTkFrame):
    ...
    def _load_image(self, img_file_or_path: Optional[str], size: Tuple[int, int]) -> Optional[CTkImage]:
        ...
        try:
            ...
            if potential_path.is_file():
                ...
                if file_path_to_load is None:
                     LOGGER.error(f"Folder.RES not defined or image '{img_file_or_path}' not found relative to it.")
                     self._image_cache[cache_key] = None # Cache the failure
                     return None

            # If no valid path was found after all checks
            if file_path_to_load is None:
                LOGGER.warning(f"Toolbar image not found: '{img_file_or_path}' (checked as-is and relative to RES/images if applicable)")
                self._image_cache[cache_key] = None # Cache the failure
                return None
            ...
            LOGGER.error(f"Failed to load or process toolbar image '{img_file_or_path}' as CTkImage: {e}", exc_info=True)
            self._image_cache[cache_key] = None # Cache the failure
            return None

    def add_button(self, text: str, img_file_or_path: Optional[str], command: Callable) -> ctk.CTkButton:
        ...
        self._button_refs[text] = btn # Use text as key (ensure uniqueness or use a different key)
    ...
    def set_img(self, btn: ctk.CTkButton, img_file_or_path: Optional[str]):
        ...
        icon_size = getattr(btn, '_image_size', self._calculate_icon_size()) # Fallback size
        ...
        try:
            ...
            original_text = btn.cget('text') # Might be empty if image was previously set
            display_text = "" if new_ctk_image else original_text # Or fetch original intended text
            if btn.cget('text') != display_text:
             ...
             LOGGER.warning(f"TclError configuring button image in set_img (widget might be destroyed): {e}")
        except Exception as e:
            LOGGER.error(f"Unexpected error configuring button image in set_img: {e}", exc_info=True)


    def add_sep(self):
        """Adds a vertical separator line to the toolbar."""
        sep_height = max(10, self.height - 8) # Make separator slightly shorter than toolbar
        # Calculate vertical padding to center the separator
        pady_sep = (self.height - sep_height) // 2
        sep = ctk.CTkFrame(self, width=2, height=sep_height, fg_color="gray50", corner_radius=1)
...
class StatusBar(ctk.CTkFrame):
    ...
    def __init__(self, master, num_columns: int, height:int = 24):
        super().__init__(master, fg_color=("gray85", "gray18"), height=height, corner_radius=0)
        if num_columns <= 0: raise ValueError("StatusBar requires at least one column")
        self.num_columns = num_columns
        self._labels: list[ctk.CTkLabel] = []
        # Cache now stores: (CTkImage | None, icon_path_or_name | None)
        self._icons: list[Tuple[Optional[CTkImage], Optional[str]]] = [(None, None)] * num_columns
    ...
    def _create_column(self, index: int):
         ...
         label = ctk.CTkLabel(self, text="", font=self._font, compound='left', anchor='w', padx=5)
         # Padding between columns, slightly less for the last one on the right
         padx_right = 2 if index < self.num_columns - 1 else 5 # More padding at the very end
    ...
    def _load_status_icon(self, icon_file_or_path: Optional[str]) -> Optional[CTkImage]:
        ...
        try:
                ...
                if hasattr(Folder, 'RES') and Folder.RES:
                    ...
                    if path_in_res.is_file():
                        ...
                        if path_in_images.is_file():
                            file_path_to_load = path_in_images
                if file_path_to_load is None:
                     LOGGER.error(f"Folder.RES not defined or status icon '{icon_file_or_path}' not found relative to it.")
            ...
            LOGGER.error(f"Failed to load status bar icon '{icon_file_or_path}' as CTkImage: {e}", exc_info=True)
            return None

    def update_column(self, index: int, text: str, icon_file_or_path: Optional[str] = None):
        ...
        new_icon_image: Optional[CTkImage] = current_icon_object # Assume no change initially
        ...
        if icon_identifier_changed:
            ...
            new_icon_image = self._load_status_icon(icon_file_or_path) if icon_file_or_path else None
        ...
        if text_changed or (new_icon_image is not current_icon_object):
             try:
                 ...
                 LOGGER.warning(f"TclError configuring status bar label {index} (widget might be destroyed): {e}")
             except Exception as e:
                 LOGGER.error(f"Unexpected error configuring status bar label {index}: {e}", exc_info=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        dummy_files = ['switch_on.png', 'switch_off.png', 'images/icon1.png', 'images/icon2.png', 'images/accept.png']
        ...
        print("Skipping dummy Folder creation - assuming common.utils.Folder is available.")
    ...
    # --- ToggleSwitch Test ---
    def toggle_action():
        state = "ON" if toggle.is_on() else "OFF" # Assuming ToggleSwitch gets an is_on() method or use internal state
        print(f"Toggle Switch state: {state}")
        # Update status bar based on toggle
        status.update_column(0, f"Toggle: {state}", "images/icon1.png" if toggle._is_on else "images/icon2.png")
    ...
    timer = Timer(timer_frame, label_text="Countdown", height=35) # label_text is currently unused internally
    ...
    app.btn2 = toolbar.add_button("Action 2", "images/icon2.png", tb_action2) # Store ref
    toolbar.add_sep()
