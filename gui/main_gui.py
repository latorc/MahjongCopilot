...
class MainGUI(ctk.CTk):
    ...
    def _setup_window(self):
        ...
        try:
            ...
            if res_folder and os.path.isdir(res_folder):
                ...
                if os.path.exists(icon_path_ico):
                    try:
                        ...
                        LOGGER.error(
                            f"设置 iconbitmap 时发生 TclError (文件 '{icon_path_ico}' 是有效的 .ico 吗?): {e_ico}",
                            exc_info=False,
                        )
                    except Exception as e_ico_gen:
                        LOGGER.error(
                            f"从 '{icon_path_ico}' 设置 iconbitmap 失败: {e_ico_gen}",
                            exc_info=True,
                        )
                else:
                ...
                if os.path.exists(icon_path_photo):
                    ...
                    LOGGER.info(
                        f"图标 '{icon_filename_photo}' 未找到，尝试使用 '{icon_filename_ico}' 作为 iconphoto 源。"
                    )
                ...
                if photo_source_path:  # 如果找到了源文件
                    try:
                        ...
                        LOGGER.error(
                            f"从 '{photo_source_path}' 加载或设置 iconphoto 失败: {e_photo}",
                            exc_info=True,
                        )
                else:
                    LOGGER.warning(
                        f"未能找到适用于 iconphoto 的图标文件 (已检查 '{icon_filename_photo}' 和备选)。"
                    )
            ...
            LOGGER.error(
                f"设置图标时发生 AttributeError (请检查 Folder/sub_file 是否正确?): {e_attr}",
                exc_info=True,
            )
    ...
    def _create_widgets(self):
        ...
        self._create_status_bars(
            self.grid_frame, cur_row
        )  # 注意这里是 cur_row，不是固定的 4
    ...
    def _create_toolbar1(self, parent, row):
        ...
        self.toolbar.add_button(
            ...
            self._on_btn_settings_clicked,
        )
        self.toolbar.add_button(
            self.st.lan().OPEN_LOG_FILE, self.icons.get("log"), self._on_btn_log_clicked
        )
        self.btn_help = self.toolbar.add_button(
            self.st.lan().HELP, self.icons.get("help"), self._on_btn_help_clicked
        )
        self.toolbar.add_sep()
        self.toolbar.add_button(
            self.st.lan().EXIT, self.icons.get("exit"), self._on_exit
        )
    ...
    def _create_toolbar2(self, parent, row):
        ...
        self.tb2.grid(
            row=row, column=0, sticky="ew", padx=PADX, pady=(PADY_INNER, PADY_INNER)
        )

        self.tb2.add_sep()
        self.switch_overlay = self._create_switch_group(
            self.tb2, self.st.lan().WEB_OVERLAY, self._on_switch_hud_toggled
        )
        self.tb2.add_sep()
        self.switch_autoplay = self._create_switch_group(
            self.tb2, self.st.lan().AUTOPLAY, self._on_switch_autoplay_toggled
        )
        self.tb2.add_sep()
        self.switch_autojoin = self._create_switch_group(
            self.tb2, self.st.lan().AUTO_JOIN_GAME, self._on_switch_autojoin_toggled
        )
        ...
        self.combo_autojoin_level = ctk.CTkComboBox(
            combo_frame,
            variable=self.auto_join_level_var,
            values=level_options,
            state="readonly",
            command=self._on_autojoin_level_selected,
        )
        ...
        self.combo_autojoin_mode = ctk.CTkComboBox(
            combo_frame,
            variable=self.auto_join_mode_var,
            values=mode_options,
            state="readonly",
            command=self._on_autojoin_mode_selected,
        )
        ...
        self.timer = Timer(
            master=self.tb2,
            label_text=self.st.lan().AUTO_JOIN_TIMER,
            height=TB2_HEIGHT - 10,
        )
    ...
    def _setup_combobox_data(
        self, get_options, get_initial_index, log_prefix
    ) -> Tuple[tk.StringVar, list]:
        ...
        try:
            ...
            if not isinstance(options, list) or not options:
                options = ["Error"]
                LOGGER.error(f"{log_prefix} options are not a valid list.")
            else:
                try:
                    ...
                    if isinstance(idx, int) and 0 <= idx < len(options):
                        ...
                        LOGGER.warning(
                            f"Invalid initial {log_prefix} index ({idx}), defaulting to 0."
                        )
                        initial_index = 0
                except Exception as e_idx:
                    LOGGER.error(
                        f"Error getting initial {log_prefix} index: {e_idx}",
                        exc_info=True,
                    )
                    initial_index = 0
        except Exception as e:
            LOGGER.error(
                f"Unexpected error initializing {log_prefix} combobox data: {e}",
                exc_info=True,
            )
            options = ["Error"]
            initial_index = 0

        if initial_index >= len(options):
            initial_index = 0
    ...
    def _get_mode_display_index(self, display_options: list) -> int:
        ...
        if initial_mode_value not in internal_modes:
            LOGGER.error(
                f"Initial mode '{initial_mode_value}' not in internal GAME_MODES list: {internal_modes}. Defaulting index."
            )
            return 0  # Default index on error
        ...
        try:
            ...
            if 0 <= mode_idx_internal < len(display_options):
                ...
                LOGGER.error(
                    f"Internal mode index {mode_idx_internal} for '{initial_mode_value}' is out of bounds for display options (len {len(display_options)}). Defaulting index."
                )
                return 0  # Default index on error
        except (
            ValueError
        ):  # Should not happen if initial_mode_value is in internal_modes
            LOGGER.error(
                f"Cannot find mode '{initial_mode_value}' in internal GAME_MODES list (this should not happen). Defaulting index."
            )
            return 0  # Default index on error
        except Exception as e:
            LOGGER.error(
                f"Unexpected error getting mode display index: {e}", exc_info=True
            )
            return 0
    ...
    def _create_ai_section(self, parent, row):
        ...
        ai_frame.grid(
            row=row, column=0, sticky="nsew", padx=PADX, pady=(PADY_INNER, PADY_INNER)
        )
        ai_frame.grid_columnconfigure(0, weight=1)
        ai_frame.grid_rowconfigure(1, weight=1)

        label_ai = ctk.CTkLabel(
            ai_frame, text=self.st.lan().AI_OUTPUT, anchor="w", font=self.header_font
        )
        label_ai.grid(
            row=0, column=0, sticky="ew", padx=PADY_INNER, pady=(PADY_INNER, 2)
        )

        self.text_ai_guide = ctk.CTkTextbox(
            ai_frame,
            font=self.ai_guide_font,
            wrap="word",
            border_width=1,
            state="disabled",
            height=AI_GUIDE_HEIGHT,
        )
        self.text_ai_guide.grid(
            row=1, column=0, sticky="nsew", padx=PADY_INNER, pady=(0, PADY_INNER)
        )
    ...
    def _create_game_info_section(self, parent, row):
        ...
        label_gi = ctk.CTkLabel(
            game_info_frame,
            text=self.st.lan().GAME_INFO,
            anchor="w",
        ...
        self.text_gameinfo.grid(
            row=1, column=0, sticky="ew", padx=PADY_INNER, pady=(0, PADY_INNER)
        )
    ...
    def _create_status_bars(self, parent, start_row):
        ...
        self.model_bar.grid(
            row=start_row, column=0, sticky="ew", padx=PADX, pady=(PADY_INNER, 0)
        )

        self.status_bar = StatusBar(parent, num_columns=3, height=STATUS_BAR_HEIGHT)
        # Use start_row + 1 for the second status bar
        self.status_bar.grid(
            row=start_row + 1, column=0, sticky="ew", padx=PADX, pady=(1, PADY_OUTER[1])
        )
    ...
    def _init_gui_state(self):
        ...
        if hasattr(self, "switch_autoplay"):
            if self.st.enable_automation:
                self.switch_autoplay.select()
            else:
                self.switch_autoplay.deselect()
        ...
        if hasattr(self, "switch_autojoin"):
            if self.st.auto_join_game:
                self.switch_autojoin.select()
            else:
                self.switch_autojoin.deselect()
        ...
        try:
            ...
            if isinstance(game_levels, list):
                try:
                    ...
                    LOGGER.debug(
                        f"自动加入等级设置为索引: {self.st.auto_join_level} ('{choice}')"
                    )
                except ValueError:
                    LOGGER.error(f"选择的等级 '{choice}' 未在 GAME_LEVELS 列表中找到。")
            else:
                LOGGER.error("语言设置中的 GAME_LEVELS 不是列表。")
        except Exception as e:
            LOGGER.error(
                f"从选择 '{choice}' 设置自动加入等级时出错: {e}", exc_info=True
            )

    def _on_autojoin_mode_selected(self, choice: str):
        try:
            game_modes_display = self.st.lan().GAME_MODES
            if not isinstance(game_modes_display, list):
                raise ValueError("显示模式 (GAME_MODES) 不是列表。")
            if "GAME_MODES" not in globals() or not isinstance(GAME_MODES, list):
                raise ValueError("内部 GAME_MODES 定义无效。")
            internal_modes = GAME_MODES
            ...
            try:
                ...
                if 0 <= mode_idx < len(internal_modes):
                    ...
                    LOGGER.debug(
                        f"自动加入模式设置为: {self.st.auto_join_mode} ('{choice}')"
                    )
                else:
                    LOGGER.error(
                        f"从 '{choice}' 得到的模式索引 {mode_idx} 对内部 GAME_MODES (长度 {len(internal_modes)}) 无效。"
                    )
            except ValueError:
                LOGGER.error(
                    f"选择的模式 '{choice}' 未在显示的 GAME_MODES 列表中找到。"
                )
        except Exception as e:
            LOGGER.error(
                f"从选择 '{choice}' 设置自动加入模式时出错: {e}", exc_info=True
            )
    ...
    def _on_switch_toggled(
        self, switch_attr: str, enable_func, disable_func, setting_attr: str
    ):
        """Generic handler for toggle switches."""
        if not hasattr(self, switch_attr):
            return
        switch_widget = getattr(self, switch_attr)
        if not switch_widget.winfo_exists():
            return
        ...
        try:
            ...
            LOGGER.error(f"切换开关 {switch_attr} 时出错: {e}", exc_info=True)
    ...
    def _on_btn_log_clicked(self):
        ...
        if not log_file:
            LOGGER.error("日志文件路径未设置。")
            messagebox.showerror("错误", "日志文件路径未配置。")
            return
        if not os.path.exists(log_file):
            LOGGER.error(f"日志文件未找到: '{log_file}'")
            messagebox.showerror("错误", f"日志文件未找到:\n{log_file}")
            return
        try:
            ...
            LOGGER.error(f"打开日志文件 '{log_file}' 失败: {e}", exc_info=True)
    ...
    def _on_btn_settings_clicked(self):
        try:
            ...
            if getattr(settings_window, "exit_save", False):
                LOGGER.info("设置已保存。")
                if getattr(settings_window, "model_updated", False):
                    self.bot_manager.set_bot_update()
                if getattr(settings_window, "gui_need_reload", False):
                    self.reload_gui()
            else:
                LOGGER.info("设置已取消或未保存关闭。")
        except Exception as e:
            LOGGER.error(f"打开或处理设置窗口时出错: {e}", exc_info=True)
            messagebox.showerror("错误", f"无法打开设置窗口:\n{e}")
    ...
    def _on_btn_help_clicked(self):
        try:
            ...
            LOGGER.error(f"打开帮助窗口时出错: {e}", exc_info=True)
            messagebox.showerror("错误", f"无法打开帮助窗口:\n{e}")
    ...
    def _on_exit(self):
        ...
        if ConfirmExitDialog.ask(
            master=self, title=title, message=message
        ):  # 调用修改后的对话框
            # --- 退出清理逻辑不变 ---
            try:
                ...
                if (
                    ...
                    and self.icons.get("yellow")
                ):
                    try:
                        exit_text = getattr(self.st.lan(), "EXITING", "正在退出...")
                        self.status_bar.update_column(
                            2, exit_text, self.icons["yellow"]
                        )
                        self.update_idletasks()
                    except Exception as e_status:
                        LOGGER.warning(f"退出时无法更新状态栏: {e_status}")

                if hasattr(self.st, "save_json") and callable(self.st.save_json):
                    self.st.save_json()
                    LOGGER.info("设置已保存。")
                else:
                    LOGGER.warning("Settings 对象缺少 save_json 方法。")

                if hasattr(self.bot_manager, "stop") and callable(
                    self.bot_manager.stop
                ):
                    LOGGER.info("正在停止 Bot Manager...")
                    self.bot_manager.stop(wait=True)
                    LOGGER.info("Bot Manager 已停止。")
                else:
                    LOGGER.warning("BotManager 对象缺少 stop 方法。")

            except Exception as e:
                LOGGER.error(f"退出清理过程中出错: {e}", exc_info=True)
            finally:
                LOGGER.info("退出 Tkinter 主循环。")
                self.quit()
        else:
            LOGGER.info("用户取消退出。")
    ...
    def _schedule_gui_update(self):
        if self.winfo_exists():
            self.after(GUI_UPDATE_INTERVAL_MS, self._update_gui_info_safe)

    def _update_gui_info_safe(self):
        try:
            if self.winfo_exists():
                self._update_gui_info_inner_optimized()
        except Exception as e:
            LOGGER.error(f"GUI 更新过程中出错: {e}", exc_info=True)
        finally:
            self._schedule_gui_update()

    def _update_widget_state(self, cache_key: str, new_value: Any, update_func) -> bool:
        if (
            cache_key not in self._cached_widget_states
            or self._cached_widget_states[cache_key] != new_value
        ):
            try:
                update_func(new_value)
                self._cached_widget_states[cache_key] = new_value
                return True
            except tk.TclError as e:
                LOGGER.warning(f"更新部件 '{cache_key}' 时发生 TclError: {e}")
                return False
            except Exception as e:
                LOGGER.warning(f"更新部件 '{cache_key}' 时出错: {e}", exc_info=True)
                return False
        return False
    ...
    def _update_gui_info_inner_optimized(self):
        ...
        try:
            ...
            is_proxy_client = (
                self.bot_manager.get_game_client_type() == GameClientType.PROXY
            )
            target_state = (
                "disabled" if (is_browser_running or is_proxy_client) else "normal"
            )
            if (
                hasattr(self, "btn_start_browser")
                and self.btn_start_browser.winfo_exists()
            ):
                self._update_widget_state(
                    "start_browser_state",
                    target_state,
                    lambda state: self.btn_start_browser.configure(state=state),
                )
        except Exception as e:
            LOGGER.warning(f"更新开始浏览器按钮状态时出错: {e}")
        ...
        try:
            ...
            if (
                ...
                and hasattr(self, "btn_help")
                and self.btn_help.winfo_exists()
            ):
                ...
                self._update_widget_state(
                    ...
                    target_icon_path,
                    lambda path: (
                        self.toolbar.set_img(self.btn_help, path) if path else None
                    ),
                )
        except Exception as e:
            LOGGER.warning(f"更新帮助按钮图标时出错: {e}")

        # --- 更新开关状态 (基于设置) ---
        switch_map = {}
        if hasattr(self, "switch_overlay"):
            switch_map["overlay_switch"] = (self.switch_overlay, self.st.enable_overlay)
        if hasattr(self, "switch_autoplay"):
            switch_map["autoplay_switch"] = (
                self.switch_autoplay,
                self.st.enable_automation,
            )
        if hasattr(self, "switch_autojoin"):
            switch_map["autojoin_switch"] = (
                self.switch_autojoin,
                self.st.auto_join_game,
            )
        for cache_key, (switch_widget, should_be_on) in switch_map.items():
            if switch_widget.winfo_exists():
                if (
                    cache_key not in self._cached_widget_states
                    or self._cached_widget_states[cache_key] != should_be_on
                ):
                    try:
                        ...
                        self._cached_widget_states[cache_key] = (
                            should_be_on  # 更新缓存为目标状态
                        )
                    except Exception as e:
                        LOGGER.warning(f"更新开关 {cache_key} 时出错: {e}")
        ...
        try:
            ...
            if pending_reaction:
                guide_base, options = mjai_reaction_2_guide(
            ...
            LOGGER.warning(f"获取 AI 指导时出错: {e}")
        if hasattr(self, "text_ai_guide") and self.text_ai_guide.winfo_exists():
            self._update_widget_state(
                "ai_guide_text",
                ai_guide_str,
                lambda text: self._update_textbox(self.text_ai_guide, text),
            )
        ...
        try:
            ...
            if gi and hasattr(gi, "my_tehai") and gi.my_tehai is not None:
                tehai = gi.my_tehai
                tsumohai = getattr(gi, "my_tsumohai", None)
                hand_str = "".join(MJAI_TILE_2_UNICODE.get(t, "?") for t in tehai)
                if tsumohai:
                    hand_str += f"   {MJAI_TILE_2_UNICODE.get(tsumohai, '?')}"
        except Exception as e:
            LOGGER.warning(f"获取游戏信息展示时出错: {e}")
        if hasattr(self, "gameinfo_var"):
            self._update_widget_state(
                "game_info_text", hand_str, lambda text: self.gameinfo_var.set(text)
            )

        # --- 更新模型信息栏 ---
        if hasattr(self, "model_bar") and self.model_bar.winfo_exists():
            self._update_model_bar_info()
        # --- 更新状态栏 ---
        ...
        try:
            ...
            LOGGER.warning(f"调用 update_overlay 时出错: {e}")
    ...
    def _update_textbox(self, textbox: ctk.CTkTextbox, new_text: str):
        ...
        try:
            ...
            LOGGER.warning(f"更新文本框内容时发生 TclError: {e}")
        except Exception as e:
            LOGGER.error(f"更新文本框内容失败: {e}", exc_info=True)
    ...
    def _update_model_bar_info(self):
        ...
        try:
            if self.bot_manager.is_bot_created() and hasattr(self.bot_manager, "bot"):
                ...
                mode_strs = [
                    (("✔" if m in supported else "✖") + m.value) for m in GameMode
                ]
                model_text = f"{self.st.lan().MODEL}: {self.st.model_type} ({' | '.join(mode_strs)})"
                model_icon_key = "green"
                if self.bot_manager.is_game_syncing():
                    info_text = "⌛ " + self.st.lan().SYNCING
                elif self.bot_manager.is_bot_calculating():
                    info_text = "⌛ " + self.st.lan().CALCULATING
                else:
                    bot_info = getattr(self.bot_manager.bot, "info_str", "")
                    info_text = ("ℹ️ " + bot_info) if bot_info else ""
            else:
                if self.bot_manager.is_loading_bot:
                    model_text, model_icon_key = self.st.lan().MODEL_LOADING, "yellow"
                else:
                    model_text, model_icon_key = self.st.lan().MODEL_NOT_LOADED, "red"
            # Update columns using cached checks
            model_icon_path = self.icons.get(model_icon_key)
            self._update_widget_state(
                "model_bar_col0",
                (model_text, model_icon_path),
                lambda val: self.model_bar.update_column(0, val[0], val[1]),
            )
            self._update_widget_state(
                "model_bar_col1",
                (info_text, None),
                lambda val: self.model_bar.update_column(1, val[0], val[1]),
            )  # Col 1 has no icon here
        except Exception as e:
            LOGGER.warning(f"更新模型信息栏时出错: {e}", exc_info=True)
    ...
    def _update_status_bar_info(self):
        ...
        try:
            ...
            self._update_widget_state(
                "status_bar_col0",
                (main_text, main_icon_path),
                lambda val: self.status_bar.update_column(0, val[0], val[1]),
            )
            ...
            if client_type == GameClientType.PLAYWRIGHT:
                ...
                browser_running = (
                    hasattr(self.bot_manager, "browser")
                    and self.bot_manager.browser.is_running()
                )
                client_icon_key = "green" if browser_running else "gray"
            elif client_type == GameClientType.PROXY:
                client_text, client_icon_key = self.st.lan().PROXY_CLIENT, "green"
            else:
                client_text, client_icon_key = self.st.lan().GAME_NOT_RUNNING, "ready"
            client_icon_path = self.icons.get(client_icon_key)
            self._update_widget_state(
                "status_bar_col1",
                (client_text, client_icon_path),
                lambda val: self.status_bar.update_column(1, val[0], val[1]),
            )

            # Col 2: Game/Error Status
            gi_status: Optional[GameInfo] = None
            try:
                gi_status = self.bot_manager.get_game_info()
            except Exception:
                pass
            ...
            self._update_widget_state(
                "status_bar_col2",
                (status_str, status_icon_path),
                lambda val: self.status_bar.update_column(2, val[0], val[1]),
            )
        except Exception as e:
            LOGGER.warning(f"更新状态栏时出错: {e}", exc_info=True)

    def _get_status_text_icon_key(
        self, gi: Optional[GameInfo]
        ...
        try:
            ...
            game_error = self.bot_manager.get_game_error()
            if bot_exception:
                return error_to_str(bot_exception, self.st.lan()), "red"
            if game_error:
                return error_to_str(game_error, self.st.lan()), "red"
            if self.bot_manager.is_browser_zoom_off():
                return self.st.lan().BROWSER_ZOOM_OFF, "red"

            if self.bot_manager.is_in_game():
                info_str = self.st.lan().GAME_RUNNING
                game_mode_value = getattr(gi, "game_mode", None)
                ...
                if self.bot_manager.is_game_syncing():
                    ...
                    bakaze_str = self.st.lan().mjai2str(gi.bakaze)
                    kyoku = getattr(gi, "kyoku", "?")
                    honba = getattr(gi, "honba", "?")
                    info_str += f"{game_mode_str} - {bakaze_str} {kyoku}{self.st.lan().KYOKU} {honba}{self.st.lan().HONBA}"
                else:
                    info_str += game_mode_str + " - " + self.st.lan().GAME_STARTING
                ...
                ui_state = getattr(
                    self.bot_manager.automation, "ui_state", UiState.NOT_RUNNING
                )
        ...
        for img_name in icon_files.values():
            ...
            if not os.path.exists(p):
                try:
                    ...
                    print(f"无法创建 dummy 图片 {p}: {e}")
    else:
        print("跳过 dummy 图片创建: Folder.RES 未设置或目录创建失败。")
