...
from PIL import (
    ImageTk,
)  # 导入 ImageTk 以便能识别 PhotoImage 类型（尽管对象是从 master 传来）
...
class ConfirmExitDialog(ctk.CTkToplevel):
    ...
    def __init__(self, master, title: str, message: str):
        if not isinstance(master, (ctk.CTk, ctk.CTkToplevel)):
            LOGGER.warning(
                f"对话框的 Master 窗口类型是 {type(master)}, 期望是 CTk 或 CTkToplevel."
            )
    ...
    def _setup_dialog(self):
        ...
        self.protocol(
            "WM_DELETE_WINDOW", self._cancel_clicked
        )  # 点击关闭按钮时调用取消
        ...
        if master_icon_path_ico and os.path.exists(master_icon_path_ico):
            try:
                ...
                LOGGER.warning(
                    f"对话框设置 wm_iconbitmap 失败: {e_wm_ico_gen}", exc_info=True
                )
        else:
            LOGGER.debug("对话框: 未从主窗口获取到有效的 .ico 路径用于 wm_iconbitmap。")

        # 2. 【延迟执行】使用 after 设置 PhotoImage (关键步骤，用于窗口左上角)
        if master_icon_photo and isinstance(
            master_icon_photo, ImageTk.PhotoImage
        ):  # 检查是否是有效的 PhotoImage 对象
            try:
                ...
                LOGGER.error(
                    f"对话框安排 iconphoto 设置失败: {e_schedule}", exc_info=True
                )
        else:
            LOGGER.debug(
                "对话框: 未从主窗口获取到有效的 PhotoImage 对象用于 iconphoto。"
            )
        # --- 图标设置结束 ---
    ...
    def _create_widgets(self):
        ...
        self.message_label.grid(
            row=0, column=0, columnspan=2, padx=20, pady=(20, 15), sticky="nsew"
        )
        ...
        ok_text = getattr(
            getattr(self._master, "st", None),
            "lan",
            lambda: type("obj", (object,), {"OK_BUTTON": "确定"}),
        )().OK_BUTTON
        cancel_text = getattr(
            getattr(self._master, "st", None),
            "lan",
            lambda: type("obj", (object,), {"CANCEL_BUTTON": "取消"}),
        )().CANCEL_BUTTON
